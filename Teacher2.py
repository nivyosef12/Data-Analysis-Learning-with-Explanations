...
class Teacher2(Teacher):
    ...
    def most_discriminative_feature(self, example, explanation, prediction, true_label):
        ...
        for i in different_indexes:
            num_of_stsfyng_exmpls_with_prediction = len({k: v for k, v in self.features_labels_dict.items() if k[i] == example[i] and v == prediction})
            num_of_stsfyng_exmpls_with_true_label = len({k: v for k, v in self.features_labels_dict.items() if k[i] == example[i] and v == true_label})
