...
if __name__ == "__main__":
    ...
    # extract input features (X) and target variable (y)
    X = np.array(df.iloc[:, :-1].values)  # extract all columns except the last one as input features (X)
    y = np.array(df.iloc[:, -1].values)  # extract the last column as the target variable (y)
